...
// @author       是非分明 x 某人
// @version      1.0.0
// @description  调用NC api 实现检测、消息预览、远程退群
// @timestamp    0
// @license      MIT
...
if (!ext) {
    ...
    seal.ext.registerStringConfig(ext, "C_url", "在这里写你的HTTP服务器地址，带上/");
    seal.ext.registerTemplateConfig(ext, "C_QQ", ["这里是用于接受的QQ号", "", "", ""]);
    seal.ext.registerTemplateConfig(ext, "C_QQ_Group", ["这里是用于接受的QQ群号", "", "", ""]);
    seal.ext.registerStringConfig(ext, "dailyExpression", "07:00", "定时任务[检查群组列表]的daily表达式", "到时间检查群组列表中有无奇怪东西。格式为 hh:mm 或者 h:mm");
    // 某：corn表达式更加可自定义。但是可能会增加学习成本，考虑加一个bool配置允许切换
    // 某：我又考虑了下还是corn好，daily太受限了
    ...
    /**
     * 检查群组名称是否包含屏蔽词并发送消息
     * 调用NC API获取群组列表，检查群名是否包含屏蔽词，如果包含则发送消息
...
全体禁言状态: ${group.group_all_shut === -1 ? '是' : '否'}`;
    ...
    /**
     * 标准化URL中的斜杠，将多个斜杠替换为单个斜杠，并移除换行符。
     * 同时验证URL格式是否正确，只允许http和https协议。
     * @param {string} str - 需要标准化的URL字符串。
     * @returns {string|null} 标准化后的URL字符串，如果格式不正确则返回null。
     */
    function T_normalizeURL(str) {
        ...
        // 检查是否为空字符串
        if (!str) {
            console.error("[url标准化]：空地址！请检查配置并重载插件！");
            return null;
        }

        // 验证URL格式，只允许http和https，免得有人才写个ws://xxx上去
        // 某：青果群有人不写http:// 越发的觉得有必要
        const urlRegex = /^https?:\/\/[\-A-Za-z0-9+&@#\/%?=~_|!:,.;]*[\-A-Za-z0-9+&@#\/%=~_|]$/i;

        if (!urlRegex.test(str)) {
            console.error("[url标准化]：地址异常！请检查配置并重载插件！");
            return null;
        }
    ...
    /**
     * 向NapCat API发起POST请求。
     * @param {string} baseurl - HTTP服务器的URL，应以斜杠结尾（虽然会自动标准化，但是这是好习惯）
     * @param {string} apipath - API的路径。
     * @param {object} body - 请求体，将被JSON.stringify转换。
     * @returns {Promise<object|null>} 请求成功则返回解析后的JSON响应数据，失败则返回null。
     */
    async function A_request_N(baseurl, apipath, body) { //这里的A代表使用API完成，后缀N代表适用于NapCat
        let Nurl = T_normalizeURL(baseurl + apipath); // 理论上baseurl应该以/结尾，不过没关系，会标准化的
        ...
        try {
            ...
            if (!response.ok) {
                ...
                console.error(`HTTP请求失败，状态码：${response.status}`, response_data);
                return null;
            }
    ...
    class numberListManager {
         ...
         * Map 的key是 Set 的名称，value是对应的 Set 对象。
         */
        constructor() {
            ...
            switch (method) {
                ...
                case 'rm':
                    ...
                    if (!targetSet.has(processedValue)) {
                        ...
                        console.info(`数字 "${processedValue}" 已成功从列表 "${setName}" 删除。`); // 这些console.info最后都要记得注释掉
                    }
                    break;
        ...
        if (response_data && response_data.data && Array.isArray(response_data.data)) {
            ...
            response_data.data.forEach(member => {
                ...
                if (member.role === "owner" || member.role === "admin") {
                    // 将Unix时间戳（秒）转换为 JavaScript Date 对象（毫秒）
                    const joinDate = new Date(member.join_time * 1000);

                    // 创建成员信息对象
                    const memberInfo = {
                        userId: member.user_id, // QQ号
    ...
    async function testApiCall(group_id) {
        ...
        const result = await A_request_N(url, "/get_group_member_list", {
            "group_id": group_id
        }); // 这里可以作为示例调用，或许apipath可以改为从参数传入。算了，反正是测试用的
        return result;
    }
	...
	async function testApiCall(group_id) {
	  console.info("正在发起 API 请求...");
	  const result = await A_request_N(url, "/get_group_member_list", {"group_id": group_id});  // 这里可以作为示例调用，或许apipath可以改为从参数传入。算了，反正是测试用的
...
async function sendForwardedHistory(historyResponse, targetGroupId) {
    ...
    // 确保 historyResponse 是一个有效的对象，并且API调用成功，包含消息数据
    ...
    for (const msg of chatMessages) {
        ...
        messagesToForward.push({
            ...
            data: {
                user_id: msg.user_id, // 消息发送者的QQ号
                nickname: msg.sender.nickname || `用户 ${msg.user_id}`, // 使用发送者昵称，若无则显示QQ号
    ...
    cmdGetGroupMsg.help = '取群聊消息 <群号> [消息数(默认20)]';
    ...
    seal.ext.registerTask(
        ...
        async (taskCtx) => {
                try {
                    console.info(`[定时任务] 执行函数checkGroupName(ctx, msg)，时间戳: ${taskCtx.now}`);
                    // 某：给我统一用 console log
                    await checkGroupName(ctx, msg);
                } catch (e) {
                    console.error(`[定时任务] 执行checkGroupName(ctx, msg)失败: ${e.message}\n${e.stack}`);
                }
            },
            "checkGroupNameDaily", // key
            "每天定时查群列表并通知" // description
    );
    // 某：虽然是写完了，但是似乎会导致卡加载，同时现在用的还是replyToSender，可能会导致意外情况，不知道怎么办
